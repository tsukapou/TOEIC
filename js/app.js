...
function showHome() {
  ...
  try {
    ...
    console.error('❌ updateBackupCard エラー:', error);
  }
  ...
  try {
    ...
    console.error('❌ updatePointsBadge エラー:', error);
  }
...
function startTest(testNumber) {
  // 💰 テスト実施可能チェック（収益化システム）
  if (window.monetizationSystem && !window.monetizationSystem.canTakeTest()) {
    const status = window.monetizationSystem.getPlanStatus();
    if (window.toastManager) {
      window.toastManager.show(
        `本日の無料テストは終了しました（${status.dailyTestCount}/${5}回）\\n\\nPremiumプランで無制限テストを楽しもう！`,
        'warning',
        5000
      );
    }
    // Premium購入モーダルを表示
    window.monetizationSystem.showPurchasePrompt('AFTER_5_TESTS');
    return;
  }
  ...
  // 早朝学習の実績チェック（NEW! 2025-12-09）
  if (typeof AchievementIntegration !== 'undefined' && typeof AchievementIntegration.checkMorningStudy === 'function') {
    AchievementIntegration.checkMorningStudy();
  }
...
function nextQuestion() {
  ...
  if (!AppState.shuffledQuestions || AppState.shuffledQuestions.length === 0) {
    ...
    if (window.ToastSystem) {
      window.ToastSystem.error('問題データが読み込まれていません。ホーム画面に戻ります。');
    } else {
...
function finishTest() {
  ...
  if (window.monetizationSystem) {
    window.monetizationSystem.recordTest();
  }
  ...
  // 📊 テスト完了を分析システムに記録（NEW! 2025-12-09）
  if (window.adminAnalytics) {
    const totalQuestions = AppState.shuffledQuestions ? AppState.shuffledQuestions.length : 30;
    const questionDetails = AppState.shuffledQuestions.map((q, index) => {
    ...
    window.adminAnalytics.recordTestCompletion({
      score: AppState.score,
      accuracy: Math.round((AppState.score / totalQuestions) * 100),
  ...
  // アダプティブ学習エンジンに記録（NEW! 2025-12-09）
  if (typeof AdaptiveLearningEngine !== 'undefined' && typeof AdaptiveLearningEngine.recordLearningSession === 'function') {
    // 問題詳細を作成
    const questionDetails = AppState.shuffledQuestions.map((q, index) => {
      const userAnswer = AppState.userAnswers[index];
      const isCorrect = userAnswer === q.answer;
      return {
        ...
        difficulty: q.difficulty,
        isCorrect: isCorrect,
        timeSpent: 0, // タイマーは個別に計測していないため0
        selectedAnswer: userAnswer,
        correctAnswer: q.answer
      };
    });
    ...
    const testResult = {
      ...
      accuracy: Math.round((AppState.score / totalQuestions) * 100),
      totalQuestions: totalQuestions,
      correctCount: AppState.score,
      timeSpent: timeInSeconds,
      questionDetails: questionDetails,
      testMode: AppState.testMode || 'standard',
      timestamp: Date.now()
    };
    
    AdaptiveLearningEngine.recordLearningSession(testResult);
    console.log('📊 アダプティブ学習エンジンに記録完了');
  }
  
  // 実績システムをチェック（NEW! 2025-12-09）
  if (typeof AchievementIntegration !== 'undefined' && typeof AchievementIntegration.onTestComplete === 'function') {
    // 問題ごとの詳細結果を作成
    const questionsResults = AppState.shuffledQuestions.map((q, index) => {
      const userAnswer = AppState.userAnswers[index];
      const isCorrect = userAnswer === q.answer;
      return {
        questionId: q.id,
        category: q.questionType || '未分類',
    ...
    const testResultData = {
      score: AppState.score,
      accuracy: AppState.score / totalQuestions, // 小数形式で渡す
      totalQuestions: totalQuestions,
  ...
  // アダプティブテストモードの終了処理
  if (AppState.testMode === 'adaptive' && typeof AdaptiveTestMode !== 'undefined' && typeof AdaptiveTestMode.finishTest === 'function') {
    AdaptiveTestMode.finishTest();
    return; // AdaptiveTestModeが結果画面を表示する
  }
...
function showResultScreen() {
  ...
  // 💰 高スコア達成時にPremium提案（800点以上）
  if (window.monetizationSystem && prediction.readingScore >= 400) {
    // 5秒後にPremium提案を表示（結果を見た後）
    setTimeout(() => {
      window.monetizationSystem.showPurchasePrompt('SCORE_800');
    }, 5000);
  }
  ...
  // 🌐 ソーシャルシェアボタンを表示（NEW! 2025-12-09）
  if (window.socialFeatures && percentage >= 70) {
    // 70%以上の好成績の場合のみシェアボタンを表示
    setTimeout(() => {
      const shareData = {
        score: score,
        total: total,
      ...
      const shareButtons = window.socialFeatures.showShareButtons('score', shareData);
      document.body.appendChild(shareButtons);
      
      // スコアを更新
      window.socialFeatures.updateScore(score);
    }, 2000);
  }
...
function startReviewTest() {
  ...
  if (typeof ReviewSystem === 'undefined') {
    ...
    if (window.ToastSystem) {
      window.ToastSystem.error('復習システムが読み込まれていません。ページをリロードしてください。');
    } else {
...
function startTodayReview() {
  if (typeof SpacedRepetition === 'undefined' || typeof ReviewSystem === 'undefined') {
    if (window.ToastSystem) {
      window.ToastSystem.warning('スペースドリピティションシステムが利用できません。');
    } else {
  ...
  if (dueQuestions.length === 0) {
    if (window.ToastSystem) {
      window.ToastSystem.success('今日復習すべき問題はありません！素晴らしいです！🎉');
    } else {
...
function showMyScorePage() {
  ...
  // 🧠 学習分析ダッシュボードの初期化（NEW!）
  if (history.length > 0 && typeof window.LearningAnalytics !== 'undefined' && typeof window.AnalyticsDashboard !== 'undefined') {
    console.log('🧠 学習分析ダッシュボード初期化開始...');
    try {
      // 分析エンジンの初期化
      if (!window.LearningAnalytics.initialized) {
      ...
      console.error('分析ダッシュボード初期化エラー:', error);
    }
  }
...
// ==================== カテゴリ別テスト機能（今日のおすすめ連動） ====================

/**
 * カテゴリ別に問題を開始
 * @param {string} category - カテゴリ名（例: "品詞問題"）
...
function startTestByCategory(category, questionCount) {
  ...
  if (categoryQuestions.length === 0) {
    if (window.ToastSystem) {
      window.ToastSystem.error(`${category}の問題が見つかりません`);
    } else {
  ...
  console.log(`  該当問題数: ${categoryQuestions.length}問`);
  
  // ランダムに指定数を抽出
  const shuffled = shuffleArray([...categoryQuestions]);
  const selectedQuestions = shuffled.slice(0, Math.min(questionCount, shuffled.length));
  
  console.log(`  選択問題数: ${selectedQuestions.length}問`);
...
function startCustomTest(questions, testName) {
  console.log(`🚀 カスタムテスト開始: ${testName} (${questions.length}問)`);
...
function startWeaknessTrainingMode() {
  if (typeof WeaknessTraining === 'undefined') {
    if (window.ToastSystem) {
      window.ToastSystem.error('苦手問題集中特訓システムが読み込まれていません。');
    } else {
  ...
  if (weakCategories.length === 0) {
    if (window.ToastSystem) {
      window.ToastSystem.info('まずはテストを受けて、あなたの弱点を分析しましょう！正答率50%未満のカテゴリが見つかると、集中特訓モードを開始できます。', 5000);
    } else {
  ...
  if (!success) {
    if (window.ToastSystem) {
      window.ToastSystem.warning('特訓用の問題が見つかりませんでした。もう少しテストを受けてデータを蓄積してください。');
    } else {
...
/**
 * デバッグ用：テストを自動完了する
 * ランダムな解答を生成して、すぐに結果画面へ遷移
 */
function debugAutoComplete() {
  console.log('🔧 デバッグ：自動完了開始...');
  ...
  for (let i = 0; i < totalQuestions; i++) {
    ...
    // 70%の確率で正解、30%の確率で不正解（リアルなテスト結果）
    let selectedAnswer;
    if (Math.random() < 0.7) {
      // 正解を選択
  ...
  console.log(`✅ ${totalQuestions}問の解答を生成しました`);
  console.log(`   正答率: ${(AppState.userAnswers.filter((ans, i) => ans === AppState.shuffledQuestions[i].correctAnswer).length / totalQuestions * 100).toFixed(1)}%`);
